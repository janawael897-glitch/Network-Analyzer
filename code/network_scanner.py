...
_IP_PROBE_ADDR = "8.8.8.8"  # NOSONAR - not an endpoint, used only to probe local routing interface

# Device-type string constants (used across fingerprinting + scan logic)
_DT_WINDOWS  = "Windows PC"
_DT_LINUX    = "Linux/Android"
_DT_APPLE    = "Apple Device"
_DT_ANDROID  = "Android Phone"
_DT_ROUTER   = "Router"
_DT_PRINTER  = "Printer"
_DT_TV       = "Smart TV"
_DT_TV_IOT   = "Smart TV/IoT"
_DT_CAMERA   = "IP Camera"
_DT_IOT      = "IoT Device"
_DT_RPIBERRY = "Raspberry Pi"
_DT_UNKNOWN  = "Unknown"
...
def get_gateway_ip():
=======
def _parse_gateway_windows(out: str) -> str:
...
def _parse_gateway_linux(out: str) -> str:
    for line in out.splitlines():
...
=======
            gw = _parse_gateway_windows(out)
        else:
            out = subprocess.check_output(["ip", "route"],
                stderr=subprocess.DEVNULL).decode(errors="ignore")
            gw = _parse_gateway_linux(out)
        if gw:
            return gw
    except Exception:
        pass
...
=======
def get_subnet_range(local_ip: str):
>>>>>>> 943b830 (Added AI IDS modules and ML pipeline)
                ...
                if ip in line:
                    for p in line.split():
                        if ":" in p and len(p) >= 11:
                            return p.upper()
=======
    # Use a thread with timeout — never touch the global socket default timeout
    # because changing it breaks Flask/SocketIO connections in other threads.
...
def _find_mac_in_parts(parts: list) -> str:
    for p in parts:
        if len(p) == 17 and ("-" in p or ":" in p):
            return p.replace("-", ":").upper()
    return ""
...
def get_mac_from_arp_cache(ip: str) -> str:
    ...
    try:
        ...
        for line in out.splitlines():
            if ip in line:
                mac = _find_mac_in_parts(line.split())
                if mac:
                    return mac
>>>>>>> 943b830 (Added AI IDS modules and ML pipeline)
    except Exception:
...
def _is_valid_arp_ip(ip: str, parts: list, subnet_prefix: str) -> bool:
    if not ip.startswith(subnet_prefix):
        return False
    last = ip.split(".")[-1]
    if last in ("0", "255"):
...
def get_full_arp_table(local_ip: str) -> list:
    """Read the OS ARP table, return only real unicast LAN devices."""
    entries = []
    subnet_prefix = ".".join(local_ip.split(".")[:3]) + "."
    try:
        out = subprocess.check_output(["arp", "-a"], stderr=subprocess.DEVNULL).decode(errors="ignore")
        for line in out.splitlines():
            ...
            ip  = parts[0].strip()
            raw = parts[1].strip()
            if not _is_valid_arp_ip(ip, parts, subnet_prefix):
                continue
            mac = _normalise_mac(raw)
            if not mac:
                continue
            if mac.startswith(("FF:FF:FF:FF:FF:FF", "01:", "33:33:")):
                continue
            entries.append({"ip": ip, "mac": mac})
    except Exception as e:
        print(f"  [ARP TABLE] Error reading ARP table: {e}")
...
=======
    print("  [UDP] Sending UDP probes to populate ARP cache ...")
>>>>>>> 943b830 (Added AI IDS modules and ML pipeline)
...
>>>>>>> 32ec94c197df011701a41008123c09d1b7019b4b
=======
# -- Per-device enrichment (runs in parallel) ----------------------------------

def _enrich_device(entry: dict, local_ip: str, gateway_ip: str) -> dict:
    ip       = entry["ip"]
    ...
    fp     = fingerprint_device(ip, hostname, vendor)
    if is_gateway and fp["device_type"] in (_DT_UNKNOWN, _DT_WINDOWS, _DT_LINUX):
        fp["device_type"] = _DT_ROUTER
        fp["icon"]        = "🌐"
        fp["confidence"]  = "High"
    if hostname:
        ...
        display_name = _DT_UNKNOWN
    return {
        "ip":          ip,
        "mac":         mac or _DT_UNKNOWN,
...
=======
    # Resolve hostnames + fingerprint in parallel (all devices at once)
    print(f"  [DNS+FP] Resolving hostnames and fingerprinting {len(found)} device(s) in parallel ...")
    import concurrent.futures, functools
    enrich = functools.partial(_enrich_device, local_ip=local_ip, gateway_ip=gateway_ip)

    devices = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(len(found), 20)) as pool:
        for device in pool.map(enrich, found):
            devices.append(device)
            flag = "  <-- THIS DEVICE" if device["is_local"] else ""
            print(f"    {device['ip']:<18} {device['mac']:<20} {device['hostname']:<30}{flag}")
>>>>>>> 943b830 (Added AI IDS modules and ML pipeline)
...
=======
    "B8:27:EB": _DT_RPIBERRY,     "DC:A6:32": _DT_RPIBERRY,
    "E4:5F:01": _DT_RPIBERRY,   "00:1A:11": "Google",
>>>>>>> 943b830 (Added AI IDS modules and ML pipeline)
...
=======
def _ttl_clues(ip: str) -> list:
    try:
        import platform, concurrent.futures as _cf
        cmd = (["ping", "-n", "1", "-w", "600", ip]
               if platform.system() == "Windows"
               else ["ping", "-c", "1", "-W", "1", ip])

        def _ping():
            return subprocess.check_output(cmd, stderr=subprocess.DEVNULL, timeout=1).decode(errors="ignore")

        with _cf.ThreadPoolExecutor(max_workers=1) as _ex:
            try:
                out = _ex.submit(_ping).result(timeout=1.5)
            except Exception:
                return []
        ...
        for part in out.split():
            if "ttl=" in part.lower():
                ...
                if ttl <= 128:
                    return [("Windows", 3)]
                if ttl >= 200:
                    return [("Router/Network", 3)]
    except Exception:
        pass
    return []


def _hostname_clues(hostname: str) -> list:
    h = (hostname or "").lower()
    if any(k in h for k in ["iphone", "apple", "mac", "macbook", "imac"]):
        return [(_DT_APPLE, 4)]
    if any(k in h for k in ["android", "samsung", "pixel", "huawei", "xiaomi", "oppo"]):
        return [(_DT_ANDROID, 4)]
    if any(k in h for k in ["windows", "desktop", "pc", "laptop", "workstation"]):
        return [(_DT_WINDOWS, 4)]
    if any(k in h for k in ["router", "gateway", "dlink", "tplink", "netgear", "asus", "linksys", "cisco", "mikrotik"]):
        return [(_DT_ROUTER, 4)]
    if any(k in h for k in ["printer", "hp", "canon", "epson", "brother"]):
        return [(_DT_PRINTER, 4)]
    if any(k in h for k in ["tv", "firetv", "roku", "chromecast", "appletv", "shield"]):
        return [(_DT_TV, 4)]
    if any(k in h for k in ["camera", "cam", "nvr", "dvr", "hikvision", "dahua"]):
        return [(_DT_CAMERA, 4)]
    if any(k in h for k in ["raspberry", "pi", "arduino", "esp", "nodemcu"]):
        return [(_DT_IOT, 4)]
    return []


def _vendor_clues(vendor: str) -> list:
    v = (vendor or "").lower()
    if "apple" in v:
        return [(_DT_APPLE, 3)]
    if "samsung" in v:
        return [(_DT_ANDROID, 3)]
    if any(k in v for k in ["intel", "realtek", "dell", "hp", "lenovo", "asus", "acer", "microsoft"]):
        return [(_DT_WINDOWS, 2)]
    if any(k in v for k in ["cisco", "netgear", "tp-link", "d-link", "zyxel", "ubiquiti", "mikrotik", "huawei"]):
        return [(_DT_ROUTER, 3)]
    if any(k in v for k in ["raspberry", "espressif", "arduino"]):
        return [(_DT_IOT, 3)]
    if any(k in v for k in ["canon", "epson", "brother", "xerox", "ricoh"]):
        return [(_DT_PRINTER, 3)]
    if any(k in v for k in ["lg", "sony", "vizio", "tcl", "hisense", "roku"]):
        return [(_DT_TV, 3)]
    return []


def _port_clues(ip: str) -> list:
    probe_ports = {9100: (_DT_PRINTER, 4), 62078: (_DT_APPLE, 4),
                   7000: (_DT_APPLE, 3),   5357:  (_DT_WINDOWS, 3),
                   1900: (_DT_TV_IOT, 2),  8080:  (_DT_ROUTER, 2)}
    clues = []
    for port, entry in probe_ports.items():
        try:
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            s.settimeout(0.3)
            if s.connect_ex((ip, port)) == 0:
...
_ICON_MAP = {
    _DT_WINDOWS: "🖥️", _DT_LINUX:   "🐧", _DT_APPLE:   "🍎",
    _DT_ANDROID: "📱", _DT_ROUTER:  "🌐", _DT_PRINTER: "🖨️",
    _DT_TV:      "📺", _DT_TV_IOT:  "📺", _DT_CAMERA:  "📷",
    _DT_IOT:     "🔌", _DT_UNKNOWN: "❓",
}


def fingerprint_device(ip: str, hostname: str, vendor: str) -> dict:
    """Identify device type via TTL, hostname, vendor, and open ports."""
    clues = (_ttl_clues(ip) + _hostname_clues(hostname)
             + _vendor_clues(vendor) + _port_clues(ip))

    device_type = _DT_UNKNOWN
    confidence  = "Low"
>>>>>>> 943b830 (Added AI IDS modules and ML pipeline)
    if clues:
        scores = {}
...
=======
    ...
    icon = _ICON_MAP.get(device_type, "❓")
>>>>>>> 943b830 (Added AI IDS modules and ML pipeline)

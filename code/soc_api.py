...
=======
# -- Auth helpers --------------------------------------------------
# Use the same cookie-based session as the rest of the app (auth.py).
...
=======
            operator=(_current_user() or {}).get("email", "admin"),
>>>>>>> 943b830 (Added AI IDS modules and ML pipeline)
        )
...
=======
        result = unblock_ip(ip, operator=(_current_user() or {}).get("email", "admin"), reason=reason)
>>>>>>> 943b830 (Added AI IDS modules and ML pipeline)
        return jsonify(result)
    except ImportError:
        return jsonify({"success": False, "error": "Firewall module not loaded"}), 500
...
=======
        ok = close_incident(incident_id, operator=(_current_user() or {}).get("email", "operator"))
>>>>>>> 943b830 (Added AI IDS modules and ML pipeline)
